...
function initPopupSystem() {
  ...
  // Show popup if never shown OR if 1 hour has passed
...
onMount(()=>{
  ...
  function onResize() {
    if (bgElem) {
      (bgElem as HTMLElement).style.marginTop = `-${(bgElem as HTMLElement).offsetHeight}px`;
    }
    if (game && game.scrollHeight == game.offsetHeight) {
      if (bgElem) (bgElem as HTMLElement).style.position = 'absolute';
    } else {
      if (bgElem) (bgElem as HTMLElement).style.position = 'relative';
    }
    const fwam = document.querySelector('.fwam');
    const fwimg = document.querySelector('.fwimg');
    if (fwam && fwimg) {
      (fwam as HTMLElement).style.marginTop = `-${(fwimg as HTMLElement).scrollHeight * 100 / 411}px`;
    }
  }
  ...
  // Initialize popup system when component mounts and game is active
  if (gameStatus !== 'idle') {
    initPopupSystem();
  }
...
// Watch for game status changes to trigger popup
          ...
          <form class="flex justify-center align-middle w-full mt-20 mb-10" action="?/cellClick" method="post" use:enhance={()=>{ isCellRequesting = true; return async ({result, update})=>{ isCellRequesting = false; await update(); if (result.data?.win) {playSound('starSnd')}; if (result.data?.loss) {endGame('lost')} }}}>
            ...
            <div class="cells relative" style='background-color: #0a0f1d;background-image: url("{minesCellsBg}");background-position: 50%;background-repeat: no-repeat;background-size: auto;border-radius: 25px;box-shadow: 0 -2px 20px rgba(42,49,69,.4),0 4px 54px rgba(42,49,69,.2);display: grid;grid-template-columns: repeat(5,66px);grid-template-rows: repeat(5,66px);padding: 10px;--cellAfter: url("{cellAfter}");--cellBefore: url("{cellBefore}")'>
              ...
              <button type="submit" name="cell" value={i} class="cell flex justify-center items-center z-[1]" on:click={()=>{playSound('btnClkSnd')}} disabled={isCellRequesting || gameStatus != 'playing' || win.indexOf(i)!=-1 || loss.indexOf(i)!=-1}>
                ...
                {:else if i % 2 == 0}
                <!-- Empty Cell SVG (same as original) -->
                ...
                {:else}
                <!-- Empty Cell SVG variant (same as original) -->
