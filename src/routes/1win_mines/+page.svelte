...
onMount(() => {
    ...
    // --- Notice Popup Logic (45 Minutes Interval) ---
    const now = Date.now();
    const lastShown = localStorage.getItem('mines_notice_last_shown');
    const fortyFiveMins = 45 * 60 * 1000; // 45 minutes in ms

    if (!lastShown || (now - parseInt(lastShown)) > fortyFiveMins) {
        showNotice = true;
        localStorage.setItem('mines_notice_last_shown', now.toString());
    ...
    return () => {
        ...
        clearTimeout(noticeTimer); // Cleanup timer
    };
})
</script>
...
<div id="main-container">
            ...
            <div class="bg-[rgb(35,45,69)] rounded-[10px] p-[2px] pl-[10px] flex items-center justify-center m-[2px] h-[calc(100%-4px)]">
                <div class="flex flex-col pr-[3px]">
                    <div class="text-[rgb(148,166,205)] flex items-center justify-center ml-auto">
                        ...
                        <svg class="fill-current" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" role="presentation" width="12" height="12">
    ...
    <!-- Notice Popup Modal -->
    {#if showNotice}
    <div class="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm transition-opacity duration-300" on:click={closeNotice}>
        <div class="bg-gray-900/80 border border-white/10 rounded-2xl p-6 max-w-sm w-full mx-4 text-center relative shadow-2xl backdrop-blur-md" on:click|stopPropagation>
            <!-- Close Button -->
            <button on:click={closeNotice} class="absolute top-3 right-3 text-gray-400 hover:text-white transition-colors">
            ...
            <div class="mb-4">
                ...
                <h3 class="text-xl font-bold text-white mb-2">Important Notice</h3>
                <p class="text-gray-300 text-sm leading-relaxed">
                    Minimum bet is <span class="text-yellow-400 font-bold">$5</span>. Please place your bet first, then click <span class="text-white font-semibold">'Get Signal'</span>. 
                    <br/><br/>
                    <span class="text-red-400">Ignoring these rules will invalidate the signal.</span>
                </p>
            </div>
            
            <!-- Timer Bar (Optional Visual) -->
            <div class="w-full bg-gray-700 h-1 rounded-full mt-4 overflow-hidden">
                <div class="bg-yellow-500 h-full rounded-full animate-[width_10s_linear_forwards]" style="width: 100%;"></div>
            </div>
            <style>
                @keyframes width {
                    from { width: 100%; }
