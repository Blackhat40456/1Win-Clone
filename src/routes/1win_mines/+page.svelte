...
// === POPUP STATE ===
$: showRulesButton = gameStatus !== 'idle';
...
function initPopupSystem() {
  ...
  const lastShown = localStorage.getItem('minesLastPopupShown');
  
  if (!lastShown || (now - parseInt(lastShown) > oneHour)) {
    showPopup();
    localStorage.setItem('minesLastPopupShown', now.toString());
  }
}
...
// === ENHANCE CALLBACK (FIXED SYNTAX) ===
const handleCellEnhance = () => {
  isCellRequesting = true;
  return async ({ result, update }: { result: any; update: any }) => {
    isCellRequesting = false;
    await update();
    if (result.data?.win) { playSound('starSnd'); }
    if (result.data?.loss) { endGame('lost'); }
  };
};

const handleBetEnhance = () => {
  gameStatus = 'waiting';
  playSound('btnClkSnd');
  return async ({ result, update }: { result: any; update: any }) => {
    gameStatus = 'playing';
...
onMount(() => {
  LoopAddReward();
  let bgElem = document.querySelector('.minesbgimg') as HTMLElement;
  ...
  function onResize() {
    ...
    if (game && game.scrollHeight == game.offsetHeight) {
      if (bgElem) bgElem.style.position = 'absolute';
    } else {
      if (bgElem) bgElem.style.position = 'relative';
    }
    const fwam = document.querySelector('.fwam') as HTMLElement;
    const fwimg = document.querySelector('.fwimg') as HTMLElement;
    if (fwam && fwimg) {
      fwam.style.marginTop = `-${fwimg.scrollHeight * 100 / 411}px`;
    }
  }
    ...
    <div class="relative h-full py-[1px] ml-auto flex">
      <div class="bg-[rgb(35,45,69)] rounded-[10px] p-[2px] pl-[10px] flex items-center justify-center m-[2px] h-[calc(100%-4px)]">
        ...
        <div class="flex items-center justify-center pl-[3px] h-full">
          <button class="text-white flex items-center rounded-[8px] cursor-pointer h-full justify-center px-[12px] text-[13px]" style="background: linear-gradient(89deg, rgb(49, 188, 105), rgb(8, 158, 78));">Deposit</button>
        </div>
      </div>
  ...
  <div class="xbl-side">
    ...
    <div class="game-container max-h-[calc(100vh-130px)] overflow-y-auto relative">
      <button on:click={function(){
        ...
        document.querySelector('.rewards')?.setAttribute('style', 'max-height: 100vh');
        document.querySelector('.rewards > .relative')?.setAttribute('style', 'max-height: calc(100vh - 85px)');
      ...
      <div class="game relative" style="background: radial-gradient(50% 50% at 50% 50%,#151b2e 0,rgba(21,27,46,0) 100%);">
        ...
        <div class="gameblocks">
          <form class="flex justify-center align-middle w-full mt-20 mb-10" action="?/cellClick" method="post" use:enhance={handleCellEnhance}>
        ...
        <div class="gamebtns flex flex-col items-center gap-y-3 mx-1.5">
          <div class="status-bar bg-[rgb(21,27,46)] rounded-2xl flex h-[60px] justify-between max-w-[510px] w-full px-4 py-2">
            ...
            <div class="multiply-list w-[18rem] relative pl-2 overflow-hidden flex gap-2 border-l-2 items-center border-[rgb(42,49,69)]">
              <div class="acl-list flex gap-2 items-center transition-all duration-300 ease-in-out" style="transform: translateX(-{multiplyActiveElem?.offsetLeft || (steps == 0 ? 0 : document.querySelector('.acl-list')?.scrollWidth || 0 - 63*5 + 8)}px);">
          ...
          <form action="?/{gameStatus == 'playing' && steps != 0 ? 'take' : 'create'}" method="post" class="bet-panel bg-[#151b2e] rounded-2xl flex justify-between max-w-[510px] w-full p-[15px] gap-2" use:enhance={handleBetEnhance}>
      ...
      <!-- Show Rules Button - Only visible when game is active -->
      {#if showRulesButton}
...
@supports (display: block) {
  :global(html) { background: #090f1e; }
  .header-top { height: 52px; padding: 0 15px; background: #090f1e; display: flex; position: relative; overflow: hidden; }
  .header-bottom { height: 47px; padding: 0 10px; margin: 0 15px; display: flex; position: relative; overflow: hidden; border-radius: 10px; background-image: linear-gradient(92deg,#1e283f,rgba(20,27,46,.6)); }
  .xbl-side { display: flex; padding: 15px; }
  .section { border-radius: 12px; display: flex; flex-direction: column; overflow: hidden; position: relative; width: 320px; height: 100%; max-height: calc(100vh - 130px); }
  .section::after {
    content: "";
    background: linear-gradient(to bottom, transparent, #141b2f);
    position: absolute; bottom: 0; left: 0; width: 100%; height: 3rem;
  }
  img { pointer-events: none; -webkit-user-drag: none; user-select: none; }
  .game-container { display: flex; width: 100%; }
  .rewards { height: 100%; max-height: calc(100vh - 130px); max-width: 400px; width: 100%; padding: 0 15px; background: #151b2e; border-radius: 16px; }
  .reward-header { display: flex; align-items: center; justify-content: between; padding: 15px 0; border-bottom: 1px solid #334155; }
  .rbutton button { border-radius: 8px; border: 2px solid transparent; color: white; font-size: 13px; font-weight: bold; line-height: 18px; padding: 9px 15px; }
  .rtable thead td { color: white; font-size: 11px; text-align: start; padding: 14px; }
  ...
  .rtable tbody td { font-size: 12px; padding: 8px; }
  .rtable tbody tr:nth-child(odd) td { background: #1b233c; }
  .rtable tbody tr:nth-child(odd) td:first-child { border-radius: 8px 0 0 8px; }
  .rtable tbody tr:nth-child(odd) td:last-child { border-radius: 0 8px 8px 0; }
  .game { width: 100%; }
  .cells::after {
...
.overflowWon::after {
  content: "You Won";
  position: absolute; width: 100%; height: 100%; top: 0; left: 0; color: #13f36c; background: inherit; text-align: center; padding-top: 8px; border-radius: inherit; transform: rotateX(30deg);
}
.overflowLost::after {
  content: "Try again";
  position: absolute; width: 100%; height: 100%; top: 0; left: 0; color: #666c8c; background: inherit; text-align: center; padding-top: 8px; border-radius: inherit; transform: rotateX(45deg);
}
@keyframes cellOut {
  from { opacity: 1; transform: scale(1); }
  to { opacity: 0.3; transform: scale(0.3); }
}
.cellOut { animation: cellOut 750ms 750ms forwards; }
.iconp::after {
...
.animateAlert { animation: 5s linear animateAlert forwards; }
.cells-board-mask {
  height: 320px;
  left: 15px;
...
@media only screen and (max-width: 1510px) {
  ...
  .game-container { display: flex; flex-direction: column; max-height: 100%; overflow: hidden; gap: 1rem; }
  .hom { display: none; }
  .gheader { right: 0; top: 8px; }
  ...
  .finalWin { position: fixed; margin-left: 0; }
}
</style>
